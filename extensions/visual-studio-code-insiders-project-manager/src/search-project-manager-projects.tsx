...
export default function Command() {
  if (!vscodeApp) {
    return ExtensionError(
      "Please configure the **Search Project Manager** Raycast extension " +
        "to choose which version of Visual Studio Code Insiders to use.",
    );
  }
  ...
  if (projectsLocationError) {
    return ExtensionError(
      "## Invalid Projects Location" +
        ...
        "\n\n" +
        "This setting should only be set if Projects Location is also customized in the VS Code Insiders Project Manager extension settings.",
    );
  }
  ...
  if (!projectEntries || projectEntries.length === 0) {
    return ExtensionError(
      "To use this extension, the VS Code Insiders Extension " +
...
function ExtensionError(detail: string) {
  ...
  return (
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          <Detail.Metadata.Label title="VS Code Insiders App" text={vscodeApp?.name || "(unset)"} />
...
function DevelopmentActionSection() {
  return environment.isDevelopment ? (
    <ActionPanel.Section title="Development">
      <Action.Open
        title="Open projects.json File in Code Insiders"
